...
=head1 DEPENDENCIES
   ...
   The MPC library is available from
    L<https://www.multiprecision.org/mpc>
...
=head1 FUNCTIONS
  ...
  (which calls 'Rmpc_clear') is automatically called on blessed
   ...
   Rmpc_set_default_prec2($p_re, $p_im);
    ...
    bits for both the real and imaginary parts.
    Rmpc_set_default_prec2 sets the default precision to be *exactly*
    $p_re bits for the real part, and *exactly* $p_im bits for the
    imaginary part.
    The precision of a variable means the number of bits used to store
    its mantissa.  All subsequent calls to `new' will use this
    precision, but previously initialized variables are unaffected.
    This is also the precision that will be used during some
    overloaded operations (see OPERATOR OVERLOADING below).
   ...
   $si = Rmpc_sub_fr($rop, $op, $mpfr, $rnd);
   $si = Rmpc_fr_sub($rop, $mpfr, $op, $rnd);
    Set $rop to 2nd arg - 3rd arg rounded in the direction $rnd.
   ...
   $si = Rmpc_fr_div($rop, $mpfr, $op, $rnd);
    Set $rop to 2nd arg / 3rd arg rounded in the direction $rnd.
    The 'sj'/'ld' versions are available only on perls built with
    '64 bit int'/'long double' support.
  ...
  Rmpc_swap($op1, $op2);
   ...
   $si = Rmpc_exp10($rop, $op, $rnd);
   $si = Rmpc_exp2($rop, $op, $rnd);
    Set $rop to (respectively) e, 10 and 2 raised to the power
    of $op, rounded according to $rnd with the precision of $rop.

   $si = Rmpc_log($rop, $op, $rnd);
   $si = Rmpc_log10($rop, $op, $rnd);
   $si = Rmpc_log2($rop, $op, $rnd);
    Set $rop to (respectively) the base e, base 10 & base 2 log
    of $op, rounded according to $rnd with the precision of $rop.
   ...
   RMPC_RE($mpfr, $op); # Deprecated - use Rmpc_realref instead.
   RMPC_IM($mpfr, $op); # Deprecated - use Rmpc_imagref instead.
   Rmpc_realref($mpfr, $op);
   Rmpc_imagref($mpfr, $op);
   ...
   FORMATTED STRING PRESENTATION

   $ui = Rmpc_printf   ( $fmt, $op); # Both real and imaginary.
   $ui = Rmpc_printf_re( $fmt, $op); # Real component only.
   $ui = Rmpc_printf_im( $fmt, $op); # Imaginary component only.
    $ui is set to the number of characters written.
    Uses mpfr_printf(). See the mpfr documentation of mpfr_printf() or
    the Math::MPFR documentation of Rmpf_printf(). As an example, if the
    precision of both the real and imaginary parts of $op is 53 bits,
    then the following will output the same as both "print $op;" and
    "print Rmpc_get_str(10, 0, $op, MPC_RNDNN) :

      Rmpc_printf( "(%.17Rg %.17Rg)", $op);

    As with Rmpfr_printf, Rmpc_printf, Rmpc_printf_re and Rmpc_printf_im
    are limited to dealing with just one object.

   ($ui1, $ui2) = Rmpc_get_str_ndigits2($base, $op); # Math-MPFR-4.1.0
                                                     # is required.
    Uses mpfr_get_str_ndigits(). See the mpfr documentation of that
    function or the Math::MPFR documentation of Rmpfr_get_str_ndigits().
    Returns the minimum number of base $base digits required to
    guarantee that the "round trip" described in the documentation will
    succeed. $ui1 is the number of digits required for the real part of
    $op, and $ui2 is the number of digits required for the imaginary part
    of $op. The values depend on the value of $base, and on the respective
    precisions of the two parts. If both parts have the same precision
    then $ui1 == $ui2.
   ...
   OPERATOR OVERLOADING
    ...
    NOTE: Also, this means that (eg) 5/$obj can be subtly different
      to "5"/$obj, as the former divides the IV 5 by $obj, while the
      latter divides the complex number (5 + 0i) by $obj - and this
      will make a difference to the sign of the imaginary component
      of the quotient if the imaginary component of $obj is 0.
      (And similarly for 2.3/$obj versus "2.3"/$obj, and the case
      $fr/$obj versus "$fr"/$obj, where $fr is a Math::MPFR object.)
    ...
    The overloading of the arithmetic operators ( +, +=, -, -=,
    /, /=, **, **= ) will accept a Math::MPFR object as the right
    hand side operand. However, if the right hand side operand is a
    Math::MPFR object and the left operand a Math::MPC object, then
    version 4.47 of Math::MPFR is needed. A Math::MPC object will be
    returned, irrespective of the order in which the two objects
    appear.

    Attempting to use the overloaded operators with objects that
    have been blessed into some package other than 'Math::MPC' or
    'Math::MPFR' will not work. The workaround is to convert this
    "foreign" object to a Math::MPC object - thus allowing it to
    work with the overloaded operator.
    ...
    5. If the variable is a Math::MPC or Math::MPFR object then the
       value of that object is used.
...
=head1 BUGS
    ...
    The mpc-1.4.0 library has buggy mpc_ui_div() and mpc_fr_div()
    implementations. Math-MPC-1.42 and later work around the issue,
    but it is recommended that mpc-1.4.0 installations be replaced
    with mpc-1.4.1 (which was released shortly after 1.4.0).
    The bug was reported and discussed at:
...
=head1 LICENSE
    ...
    Copyright 2006-2011, 2014, 2016, 2018, 2020, 2022-26 Sisyphus
