...
# if MPC_VERSION < 66304 /* less than version 1.3.0 */
  ...
  croak("Invalid rounding value (%d) supplied to Rmpc_mul_sj()", (int)rnd);
# endif
...
# if MPC_VERSION < 66304 /* less than version 1.3.0 */
  if(!_check_rounding_value(rnd))
  croak("Invalid rounding value (%d) supplied to Rmpc_mul_ld()", (int)rnd);
# endif
...
# if MPC_VERSION < 66304 /* less than version 1.3.0 */
  if(!_check_rounding_value(rnd))
  croak("Invalid rounding value (%d) supplied to Rmpc_mul_d()", (int)rnd);
# endif
...
# if MPC_VERSION < 66304 /* less than version 1.3.0 */
  if(!_check_rounding_value(rnd))
  croak("Invalid rounding value (%d) supplied to Rmpc_div_sj()", (int)rnd);
# endif
...
int Rmpc_sj_div (mpc_ptr rop, intmax_t i, mpc_ptr op, mpc_rnd_t rnd) {
   ...
   mpfr_t x;
   int inex;
   DIV_CORRECTION_DECL(op) /* Defined (and documented) in math_mpc_include.h */
   ...
   DIV_CORRECTION_POST(rop)    /* Defined (and documented) in math_mpc_include.h */
...
# if MPC_VERSION < 66304 /* less than version 1.3.0 */
  if(!_check_rounding_value(rnd))
  croak("Invalid rounding value (%d) supplied to Rmpc_div_ld()", (int)rnd);
# endif
...
# if MPC_VERSION < 66304 /* less than version 1.3.0 */
  if(!_check_rounding_value(rnd))
  croak("Invalid rounding value (%d) supplied to Rmpc_div_d()", (int)rnd);
# endif
...
# if MPC_VERSION < 66304 /* less than version 1.3.0 */
  if(!_check_rounding_value(rnd))
  croak("Invalid rounding value (%d) supplied to Rmpc_d_div()", (int)rnd);
# endif
...
SV * _mpfr_set_NV(pTHX_ mpfr_t * p, SV * q, unsigned int round) {
     ...
     return newSViv(mpfr_set_float128(*p, (float128)SvNVX(q), (mpc_rnd_t)round));
     ...
     returned = mpfr_set_str(*p, buffer, 10, (mpc_rnd_t)round);
     Safefree(buffer);
     ...
     return newSViv(mpfr_set_ld(*p, (long double)SvNVX(q), (mpc_rnd_t)round));
...
#else
     PERL_UNUSED_ARG4(mpc, ld1, ld2, round);
...
#else
     PERL_UNUSED_ARG4(mpc, d, ui, round);
...
#else
     PERL_UNUSED_ARG4(mpc, d, si, round);
...
#else
     PERL_UNUSED_ARG4(mpc, d, mpfr, round);
...
#else
     PERL_UNUSED_ARG4(mpc, mpfr, ui, round);
...
#else
     PERL_UNUSED_ARG4(mpc, mpfr, si, round);
...
#else
     PERL_UNUSED_ARG4(mpc, si, ui, round);
...
#else
     PERL_UNUSED_ARG4(mpc, mpf, uj, round);
...
#else
     PERL_UNUSED_ARG4(mpc, mpq, uj, round);
...
#else
     PERL_UNUSED_ARG4(mpc, mpz, uj, round);
...
#else
     PERL_UNUSED_ARG4(mpc, mpf, sj, round);
...
#else
     PERL_UNUSED_ARG4(mpc, mpq, sj, round);
...
#else
     PERL_UNUSED_ARG4(mpc, mpz, sj, round);
...
#else
     PERL_UNUSED_ARG4(mpc, mpf, ld, round);
...
#else
     PERL_UNUSED_ARG4(mpc, mpq, ld, round);
...
#else
     PERL_UNUSED_ARG4(mpc, mpz, ld, round);
...
#else
     PERL_UNUSED_ARG4(mpc, uj1, uj2, round);
...
#else
     PERL_UNUSED_ARG4(mpc, sj1, sj2, round);
...
int Rmpc_ui_div(pTHX_ mpc_t * a, SV * b, mpc_t * c, SV * round) {
     int inex;
     DIV_CORRECTION_DECL(*c) /* Defined (and documented) in math_mpc_include.h */
     DIVISION_BUG_DECL       /* Defined (and documented) in math_mpc_include.h */
     ...
     DIV_CORRECTION_POST(*a) /* Defined (and documented) in math_mpc_include.h */
     DIVISION_BUG_POST         /* Defined (and documented) in math_mpc_include.h */
...
#else
     PERL_UNUSED_ARG4(a, b, pow, round);
...
SV * Rmpc_mul_2ui(pTHX_ mpc_t * a, mpc_t * b, SV * c, SV * round) {
     CHECK_ROUNDING_VALUE(round);
#if MPC_VERSION < 65536 /* less than version 1.0.0 */
...
SV * overload_div(pTHX_ mpc_t * a, SV * b, SV * third) {
     ...
     SV * obj_ref, * obj;
     mpfr_t t;
     DIV_CORRECTION_DECL(*a) /* Defined (and documented) in math_mpc_include.h */
     DIVISION_BUG_DECL   /* Defined (and documented) in math_mpc_include.h */
...
#  endif
       if(SWITCH_ARGS) {
         ...
         DIV_CORRECTION_POST(*mpc_t_obj)    /* Defined (and documented) in math_mpc_include.h */
         DIVISION_BUG_POST                  /* Defined (and documented) in math_mpc_include.h */
       }
...
void overload_copy(pTHX_ mpc_t * p, SV * second, SV * third) {
     ...
     SV * obj_ref, * obj;
     mp_prec_t re, im;
     PERL_UNUSED_ARG3(items, second, third);
...
SV * Rmpc_dot(pTHX_ mpc_t * rop, SV * avref_A, SV * avref_B, SV * len, SV * round) {
     CHECK_ROUNDING_VALUE(round);
#if defined(MPC_VERSION) && MPC_VERSION >= 65793 /* not less than version 1.1.1 */
     mpc_ptr *p_A, *p_B;
     SV ** elem;
     int ret;
     unsigned long i, s = (unsigned long)SvUV(len);

     if(s > (unsigned long)av_len((AV*)SvRV(avref_A)) + 1 || s > (unsigned long)av_len((AV*)SvRV(avref_B)) + 1)
...
SV * Rmpc_sum(pTHX_ mpc_t * rop, SV * avref, SV * len, SV * round) {
     ...
     if(s > (unsigned long)av_len((AV*)SvRV(avref)) + 1)croak("2nd last arg to Rmpc_sum is greater than the size of the array");
