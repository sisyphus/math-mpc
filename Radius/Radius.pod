...
=head1 FUNCTIONS
   ...
   @parts = Rmpcr_split($op);
    ...
    the diagnostic recommends using Rmpcr_split_mpfr (below) instead.

  @parts = Rmpcr_split_mpfr($op);
    The same as Rmpcr_split, except that the numeric values returned
    are Math::MPFR objects with 64-bit precision, not IV or UV.
    This avoids the problem of having 64-bit values overflow the 4-byte
    IV or UV.
    If $op is zero, @parts is a one-element array, containing a
    Math::MPFR object with value zero.
    If $op is Inf, @parts is a one-element array, containing the string
    "Inf".
    Otherwise, @parts is a two-element array. The first element is a
    Math::MPFR object holding the value of the mantissa of $op, and the
    second element is a Math::MPFR object holding the value of the
    exponent of $op.
